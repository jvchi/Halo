// Behaviour ported from reference/aave-clone/scripts/clone.js (the clone re-adds
// the interactivity Next.js stripped). Same logic; wrapped so React can run it on
// mount and clean up the nodes it injects. Operates on the embedded reference DOM.
...
export function setupClone() {
  ...
  function setupDesktopMenus() {
    ...
    if (!nav || nav.dataset.cloneBound) return;
    nav.dataset.cloneBound = "1";
  ...
  function setupMobileMenu() {
    ...
    if (!button || !menu || button.dataset.cloneBound) return;
    button.dataset.cloneBound = "1";
  ...
  function setupFaq() {
    qa(".styles_collapsible__aqKSz").forEach((item) => {
      ...
      if (!button || !wrapper || !content || button.dataset.cloneBound) return;
      button.dataset.cloneBound = "1";
  ...
  function setupNewsletter() {
    ...
    if (!input || !button || button.dataset.cloneBound) return;
    button.dataset.cloneBound = "1";
  ...
  function setupDownloadFeedback() {
    ...
    if (!button || button.dataset.cloneBound) return;
    button.dataset.cloneBound = "1";
  ...
  // Cleanup for React unmount / StrictMode re-run.
  return () => {
